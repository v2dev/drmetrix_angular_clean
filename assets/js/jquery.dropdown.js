...
var airings_data = [];
  var settings = {
    ...
    limitCountAddAllErrorMessage: 'There is a 100 limit for '+airings_data['tabList']+'s chosen. There are too many results to use ADD ALL.',
  ...
  var triggered_once = 0;
  var ALERT_TIMEOUT_PERIOD = 1000;

  function createTemplate() {
    var isLabelMode = this.isLabelMode;
    var searchable = this.config.searchable;
    var templateSearch = searchable ? '<span class="dropdown-search user-define-class">' + this.config.input + '</span>' : '';
  ...
  function maxItemAlert(limitMessage) {
    var _dropdown = this;
    var _config = _dropdown.config;
    var $el = _dropdown.$el;
    var $alert = $el.find('.dropdown-maxItem-alert');
    var alertMessage = limitMessage;
    ...
    $('.dropdown-mul-1').animate({scrollTop: -$('.dropdown-mul-1').prop("scrollHeight")});
    $('.dropdown-display-label').before($alert);
  ...
  function getDropDownChoseListData() {
    var selectDropDownValue = [];
      $(".dropdown-chose-list").find('span.dropdown-selected').each(function(){
        selectDropDownValue.push({
  ...
  var action = {
    ...
    search: throttle(function (event) {
      ...
      $("#search_searchable_dropdown").val( $("#search_searchable_dropdown").val().trim() );
      var intputValue = $("#search_searchable_dropdown").val();
      ...
      var $alert = $('.dropdown-display-label').find('.dropdown-maxItem-alert');
      var alertMessage = _config.searchTextLengthErrorMessage;
  
      if(intputValue.length < 3 && intputValue.length != '')  {
        $('#select_all').hide();
        $alert = $('<div class="dropdown-maxItem-alert">' + alertMessage + '</div>');
        if($('.dropdown-display-label').find('.dropdown-maxItem-alert').length >= 1) {
          $('.dropdown-display-label').find('.dropdown-maxItem-alert').show();
        } else {
          $('.dropdown-display-label').before($alert);
        }
        return;
      } else {
        $('.dropdown-display-label').find('.dropdown-maxItem-alert').remove();
      }
    
      $.each(data, function (key, value) {
        if ((value.groupName && value.groupName.toLowerCase().indexOf(intputValue.toLowerCase()) > -1) || value.name.toLowerCase().indexOf(intputValue.toLowerCase()) > -1 || '' + value.id === '' + intputValue || (value.alt_name != null && value.alt_name.toLowerCase().indexOf(intputValue.toLowerCase()) > -1 ) ) {
          result.push(value);
        }
      });
     
      $el.find('ul').html(selectToDiv(objectToSelect.call(_dropdown, result)[0]) || _config.searchNoData);
      if(intputValue != '' && airings_data['no_results'] == 0)  {
        $('#select_all').show();
      }
      if(intputValue != '' && airings_data['no_results'] == 1)  {
        $('#select_all').hide();
      }
    ...
    control: function (event) {
      ...
      var itemIndex;
      var $items;
      if ((keyCode === KC.down || keyCode === KC.up) && (keyCode != KC.enter && $('#search_searchable_dropdown').val() != ''))  {
        // 方向
    ...
    selectAll: function(event, status) {
      var _dropdown = this;
      var _config = _dropdown.config;
      ...
      var selectedProp;
      var toBeAdded = [];
      var selectListAmount = $('.dropdown-selected').length;
      
      if(airings_data['list_edit_save']  == 1) {
        selectListAmount = 0;
      }
      var maxAlertcall = 0;
      airings_data['list_edit_save'] = 0;
      $('#saveUpdatedList').removeAttr('disabled');
        var selectDropDownValue = getDropDownChoseListData();
          $(".dropdown-main ul").find('.dropdown-option').not('.dropdown-chose').each(function(){
              if (selectListAmount < _config.limitCount && maxAlertcall == 0) {
              toBeAdded.push($(this).attr('data-value'));
          ...
          $.each(_config.data, function (key, item) {
            if(jQuery.inArray( item.id, toBeAdded) != -1) {
              selectedProp = item;
              item.selected = hasSelected ? false : true;
              if(item.selected == false) selectedName.push(item.name);
            }
          });
          var selectDropDownValueArr = [];
        selectDropDownValueArr = selectDropDownValue; // selected chosen value plus add all values will ve pushed to this array
          $(".dropdown-chose").each(function(){
            const result = selectDropDownValueArr.find( ({ name }) => name === $(this).text() );
            if (!result ){
              selectDropDownValueArr.push({
        ...
        _dropdown.name = [];
        if(selectDropDownValueArr) {
           $.each(selectDropDownValueArr, function (key, item) {
          ...
          $('#selected_list_count').text(selectDropDownValueArr.length);
          _config.choice.call(_dropdown, event, selectedProp);
          if(maxAlertcall == 1) {
            maxItemAlert.call(_dropdown,_config.limitCountAddAllErrorMessage);
            return false;
          }
    },
    multiChoose: function (event, status) {
      ...
      airings_data['list_edit_save'] = 0;
      $('#saveUpdatedList').removeAttr('disabled');
      // if($('#search_searchable_dropdown').val() != '') {  $('#select_all').show(); }
        ...
        if (hasSelected) {
          ...
          var selectedId = $target.attr('data-value');
          $('.dropdown-chose-list').find('[data-id='+selectedId+']').parent().remove();
          // $target.removeClass('dropdown-selected');
          selectListAmount--;
        } else {
          if (selectListAmount < _config.limitCount) {
            $target.addClass('dropdown-chose');
           selectListAmount++;
          } else {
            maxItemAlert.call(_dropdown,_config.limitCountErrorMessage);
            return false;
          }
        }
        ...
        var selectDropDownValueArr = selectDropDownValue;
       $(".dropdown-chose").each(function(){
            const result = selectDropDownValueArr.find( ({ name }) => name === $(this).text() );
            if (!result ){
              selectDropDownValueArr.push({
        ...
        if(selectDropDownValue) {
           $.each(selectDropDownValue, function (key, item) {
            _dropdown.name.push('<span class="dropdown-selected">' + item.name + '<i class="del" data-id="' + item.id + '"></i></span>');
          });
        }
        ...
        if (hasSelected && selectListAmount < _config.minCount) {
          minItemsAlert.call(_dropdown);
        }
        $('#selected_list_count').text(selectListAmount);
    ...
    del: function (event) {
      ...
      selectListAmount--;
      if(selectListAmount == 0) {
        $('#saveUpdatedList').attr('disabled','disabled');
      }
      $('#selected_list_count').text(selectListAmount);
  ...
  Dropdown.prototype = {
    ...
    renderSelect: function (isUpdate, isCover) {
      ...
      $('#selected_list_count').text(_this.name.length);
    },
    bindEvent: function () {
      $("#search_searchable_dropdown").val( $("#search_searchable_dropdown").val().trim() );
      var intputValue = $("#search_searchable_dropdown").val();
      ...
      if (_this.isLabelMode) {
        ...
        if (_this.config.searchable) {
          $el.on(EVENT_SPACE.click, '#search_list', $.proxy(action.show, _this));
        } else {
      ...
      $el.on(EVENT_SPACE.click, '#search_list', $.proxy(action.search, _this));
      $el.on(EVENT_SPACE.click, '#clearIconList', function (event) {
          $('#search_searchable_dropdown').val('');
          $('#clearIconList').hide();
          $('#select_all').hide();
      ...
      $el.on(EVENT_SPACE.keydown, 'input', function (event) {
       if ((event.keyCode === 13  &&  intputValue != '')) {
          $('.dropdown-single,.dropdown-multiple,.dropdown-multiple-label').removeClass('active');
        }
      });
      $el.on(EVENT_SPACE.keyup, function (event) {
        var keyCode = event.keyCode;
        var KC = KEY_CODE;
        if (keyCode === KC.enter && $('#search_searchable_dropdown').val() != '') {
          $el.find('#search_list').removeAttr('disabled');
          $el.find('#search_list').trigger('click');
        }
      });
       ...
       if(triggered_once == 0) {
        triggered_once = 1;
          $("#select_all").on('click', function(event){
              $.proxy( action.selectAll, _this, event)();
          });
        }
    },
    unbindEvent: function () {
      ...
      $el.off(EVENT_SPACE.click, '#clearIconList');
