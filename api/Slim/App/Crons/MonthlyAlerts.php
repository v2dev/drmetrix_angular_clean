 ...
 * Date: 15-01-2015
 * Purpose: Email will sent for monthly alerts.
 */
set_time_limit(0);
ini_set('max_execution_time', 0);

class Slim_App_Crons_MonthlyAlerts {
    private $db = NULL;
    ...
    public function __construct() {
       ...
       // $this->date = '2015-07-06';
        ...
        $cronJob = new Slim_App_Crons_HighestProjectedScoreMonth();
        $this->last_week = $cronJob->getLastMediaMonth(); 
        $this->previous_week = $this->getPreviousMediaMonth(); // previous week of current week
    }
    
     public function retrieveData(){
        $sql = "SELECT * FROM tracking WHERE status='active' AND notification_type LIKE '%email%' AND frequency='monthly'";
     ...
     public function getPreviousMediaMonth(){
        $previous_week['sd'] = date('Y-m-d',(strtotime ( '-1 month' , strtotime ( $this->last_week['sd']) ) ));
        $previous_week['ed'] = date('Y-m-d',(strtotime ( '-1 month' , strtotime ( $this->last_week['ed']) ) ));
        return $previous_week;
    }
    
    public function createEmailTemplate($result_tracking){
        $email = array();
        $fetchUser = "SELECT user_id FROM tracking group by user_id";
        ...
        foreach($users as $u1 => $u2){
          ...
          foreach($result_tracking as  $k=>$v){
           ...
           if(!empty($brand_array) || !empty($category_array) || !empty($brand_short_ranking_array) || !empty($brand_long_ranking_array) || !empty($adv_short_ranking_array) || !empty($adv_long_ranking_array)){
              $middle_content = 'Here is a monthly alert update for '.date("M", strtotime($this->last_week['sd'])).' '. date("Y", strtotime($this->last_week['ed'])).'<br>';
              ...
              $this->html.= '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
                ...
                <head>
                  ...
                  <title>Monthly Alert Report</title>
                  <style type="text/css">
    ...
    function addEntry($email,$user_id,$category){
      ...
      foreach($arr as $k => $v){
        $sql = "INSERT INTO configuration (username,user_id,type,category,triggered_on,frequency,send_mail) VALUES ('".$email."','".$user_id."','weekly alert','".$v."','".$_SESSION['triggered_on']."','weekly','".$_SESSION['sent']."')";
        $stmt = $this->db->prepare($sql);
        $stmt->execute();
      }
    }
    ...
    public function sendDailyReport($email,$name){
      $subject = 'Monthly Tracking Alert Report';
    ...
    public function rankingReport($brand_ranking_array,$type,$brand_adv_flag,$u2){
      if(!empty($brand_ranking_array)){
        ...
        $names = array();
         ...
         $rankIncreasesFind = "SELECT id,rank_increases FROM tracking WHERE ".$where." AND status = 'active' AND frequency='monthly' AND rank_increases!=0  AND form ='".$type."' AND user_id=".$u2;
          $stmt = $this->db->prepare($rankIncreasesFind);
          $stmt->execute();
          $rank_increases = $stmt->fetchAll(PDO::FETCH_ASSOC);
          
         $rankDecreasesFind = "SELECT id,rank_decreases FROM tracking WHERE ".$where." AND status = 'active' AND frequency='monthly' AND rank_decreases!=0 AND form ='".$type."' AND  user_id=".$u2;
        ...
        $sql = "SELECT brand_adv_id,ranking FROM brand_adv_spend_index WHERE brand_adv_flag = ".$brand_adv_flag." AND week_month = 2 AND brand_adv_id IN (".$ranking_ids.") AND type ='".$type."' AND from_date = '".$this->previous_week['sd']."' AND to_date = '".$this->previous_week['ed']."'";
        $stmt = $this->db->prepare($sql);
        $stmt->execute();
        ...
        $ranking_ids = implode(",",$brand_ranking_array);
        $sql = "SELECT brand_adv_id,ranking FROM brand_adv_spend_index WHERE brand_adv_flag = ".$brand_adv_flag." AND week_month = 2 AND brand_adv_id IN (".$ranking_ids.") AND type ='".$type."' AND from_date = '".$this->last_week['sd']."' AND to_date = '".$this->last_week['ed']."'";
        $stmt = $this->db->prepare($sql);
        $stmt->execute();
          ...
          foreach($brand_last_ranking as $k2 => $v2){
            if($v1['brand_adv_id'] == $v2['brand_adv_id']){
              /*if($v2['ranking'] == $v1['ranking']){
                  $brand_adv[$v1['brand_adv_id']]['diff'] = '-';
                  $brand_adv[$v1['brand_adv_id']]['type'] = 'no change';
