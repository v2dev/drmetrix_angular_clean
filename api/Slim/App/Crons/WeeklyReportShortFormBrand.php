...
class Slim_App_Crons_WeeklyReportShortFormBrand {
    private $db = NULL;
    ...
    public function retrieveData(){
          $sql = "SELECT b.brand_id as ID,SUM(a.rate) projected_score,a.length,count(a.creative_id) no_of_airings,b.brand_name,count(a.network_code) as network,sum(a.length) as duration,c.video FROM brand b INNER JOIN creative c ON b.brand_id = c.brand_id INNER JOIN airings a ON a.creative_id = c.creative_id WHERE  a.length <= 120 AND a.end >= '".$this->last_week['sd']." 00:00:00' AND a.end <= '".$this->last_week['ed']." 00:00:00' GROUP BY b.brand_id ";
        $stmt = $this->db->prepare($sql);
         if ($stmt->execute()) {
    ...
    public function retrievePreviousWeek(){
        $sql = "SELECT brand_id,current_week FROM report_brand_short_form WHERE from_date >= '".$this->previous_week['sd']."' AND from_date <= '".$this->previous_week['ed']."'";
        $stmt = $this->db->prepare($sql);
         if ($stmt->execute()) {
            $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
     ...
     public function updatePreviousWeekRanking($result){
        if(!empty($result)){
             foreach($result as  $key=>$value){
                   $updatePreviousWeekRanking = "UPDATE report_brand_short_form SET previous_week = '".$value['current_week']."' WHERE  brand_id = '".$value['brand_id']."'";
                   $stmt = $this->db->prepare($updatePreviousWeekRanking);
                   $stmt->execute();
             }
        }else{
            $updatePreviousWeekRanking = "UPDATE report_brand_short_form SET previous_week = '-'";
            $stmt = $this->db->prepare($updatePreviousWeekRanking);
            $stmt->execute();
        }
    ...
    public function retrieveMonthlyData(){
        $sql = "SELECT brand_adv_id,ranking FROM monthly_spend_index_ranking WHERE  brand_adv_flag = 1 AND type =1 AND from_date >= '".$this->monthly['sd']."' AND from_date <= '".$this->monthly['ed']."'";
        $stmt = $this->db->prepare($sql);
         if ($stmt->execute()) {
            $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
    ...
    public function updateMonthlyRanking($result){
        if(!empty($result)){
             foreach($result as  $key=>$value){
                   $updateMonthlykRanking = "UPDATE report_brand_short_form SET monthly = '".$value['ranking']."' WHERE  brand_id = '".$value['brand_adv_id']."'";
                    $stmt = $this->db->prepare($updateMonthlykRanking);
                    $stmt->execute();
             }
        }else{
            $updateMonthlykRanking = "UPDATE report_brand_short_form SET monthly = '-'";
            $stmt = $this->db->prepare($updateMonthlykRanking);
            $stmt->execute();
        }
    ...
    public function retrieveNationalAirings(){
        $sum = 0;
	$sql = "SELECT count(a.creative_id) no_of_airings,b.brand_id FROM brand b INNER JOIN creative c  ON b.brand_id = c.brand_id INNER JOIN airings a ON a.creative_id = c.creative_id WHERE a.length <= 120 AND a.breaktype='N'  AND a.end >= '".$this->last_week['sd']." 00:00:00' "
                . "AND a.end <= '".$this->last_week['ed']." 00:00:00' GROUP BY b.brand_id";
        $stmt = $this->db->prepare($sql);
        if ($stmt->execute()) {
            $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
             foreach($result as $key => $value){
                $totalAirings = Slim_App_Lib_Common::totalAiringsBrand($this->db,$this->last_week,$value['brand_id']);    
    ...
    public function updateNationalAirings($result){
        if(!empty($result)){
             foreach($result as  $key=>$value){
                   $updateNational = "UPDATE report_brand_short_form SET national_total_airings = '".$value['percent_national']."', local_total_airings = '".$value['percent_local']."' WHERE  brand_id = '".$value['brand_id']."'";
    ...
    public function calculateSpendIndex($highestScore,$value){
        $spendIndex = 0;
        if($highestScore!=0){
            $spendIndex =  round((($value['projected_score'] * 100)/$highestScore),2);
        }
        return $spendIndex;
    }
    ...
    public function addToTable($data){
        ...
        $sql =  "INSERT INTO report_brand_short_form (brand_id,from_date,to_date,spend_index,total_airings,brand_name,price,network,asd,video_id) VALUES ('".$data['ID']."','".$this->last_week['sd']."','".$this->last_week['ed']."','".$data['spend_index']."','".$data['no_of_airings']."','".addslashes(trim($data['brand_name']))."','".$data['projected_score']."','".$data['network']."','".$asd."','".$data['video']."')";
       $stmt = $this->db->prepare($sql);
       $stmt->execute();
    }
     ...
     public function updateRanking(){
        $sql = "SELECT * FROM report_brand_short_form WHERE from_date = '".$this->last_week['sd']."' AND to_date = '".$this->last_week['ed']."' ORDER BY  price DESC";
        $stmt = $this->db->prepare($sql);
       if ($stmt->execute()) {
        ...
        foreach($rankingBrands as $key => $value){
              if($value['spend_index']!=0){
                $updateRanking = "UPDATE report_brand_short_form SET current_week = '".($key+1)."' WHERE  brand_id = '".$value['brand_id']."'";
                $stmt = $this->db->prepare($updateRanking);
                $stmt->execute();
              }else{
                $updateRanking = "UPDATE report_brand_short_form SET current_week = '-' WHERE  brand_id = '".$value['brand_id']."'";
                $stmt = $this->db->prepare($updateRanking);
                $stmt->execute();
    ...
    function updateAdvertiser(){
        $sql = "SELECT a.adv_id,r.brand_id,a.company_name FROM advertiser a  INNER JOIN report_brand_short_form r ON r.adv_id = a.adv_id";
        $stmt = $this->db->prepare($sql);
        $stmt->execute();
        $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
        if(!empty($result)){
            foreach($result as $k=>$v){
              $updateAdvertiserQuery = "UPDATE report_brand_short_form SET adv_id='".$v['adv_id']."', advertiser_name = '".addslashes(trim($v['company_name']))."' WHERE  brand_id = '".$v['brand_id']."'";
    ...
    function getShortBrands(){
        $report = array();
        $sql = "SELECT * FROM report_brand_short_form WHERE from_date = '".$this->last_week['sd']."' AND to_date = '".$this->last_week['ed']."' ORDER BY  price DESC LIMIT 50";
        $stmt = $this->db->prepare($sql);
        $stmt->execute();
        $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
        if(!empty($result)){
    ...
    function convertToPdf($html){
        ...
        $file_to_save = '../assets/pdf/Drmetrix_weekly_brands_shortform_'.date("jS_M", strtotime($this->last_week['sd'])).'.pdf';
        file_put_contents($file_to_save, $output);

    }
    
    function sendWeeklyReport(){
        $file_name = 'Drmetrix_weekly_brands_shortform_'.date("jS_M", strtotime($this->last_week['sd'])).'.pdf';
        $type = 'Brand';
        $form = 'Short Form';
