...
class Slim_App_Crons_WeeklyReportLongFormAdv {
    private $db = NULL;
    ...
    public function retrieveData(){
          $sql = "SELECT b.adv_id as ID,SUM(rate) projected_score,a.length,count(a.creative_id) no_of_airings,count(a.network_code) as network,sum(a.length) as duration FROM airings a, brand b, creative c WHERE a.creative_id = c.creative_id and b.brand_id = c.brand_id AND a.end BETWEEN '".$this->last_week['sd']." 00:00:00' AND '".$this->last_week['ed']." 00:00:00' AND a.length > 120 AND b.adv_id IS NOT NULL AND b.adv_id != 0 GROUP BY b.adv_id ";
        $stmt = $this->db->prepare($sql);
         if ($stmt->execute()) {
    ...
    public function retrievePreviousWeek(){
        $sql = "SELECT adv_id,current_week FROM report_adv_long_form WHERE from_date BETWEEN '".$this->previous_week['sd']."' AND '".$this->previous_week['ed']."'";
        $stmt = $this->db->prepare($sql);
         if ($stmt->execute()) {
            $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
     ...
     public function updatePreviousWeekRanking($result){
        if(!empty($result)){
             foreach($result as  $key=>$value){
                   $updatePreviousWeekRanking = "UPDATE report_adv_long_form SET previous_week = '".$value['current_week']."' WHERE  adv_id = '".$value['adv_id']."'";
                   $stmt = $this->db->prepare($updatePreviousWeekRanking);
                   $stmt->execute();
             }
        }else{
            $updatePreviousWeekRanking = "UPDATE report_adv_long_form SET previous_week = '-'";
            $stmt = $this->db->prepare($updatePreviousWeekRanking);
            $stmt->execute();
        }
    ...
    public function retrieveMonthlyData($result){
        foreach($result as $key=>$value){
             $sql = "SELECT brand_adv_id,ranking FROM monthly_spend_index_ranking  WHERE  brand_adv_id= '".$value['ID']."' AND brand_adv_flag = 1 AND type = 1";
            $stmt = $this->db->prepare($sql);
             if ($stmt->execute()) {
    ...
    public function updateMonthlyRanking($result,$adv_id){
        if(!empty($result)){
            $updateMonthlykRanking = "UPDATE report_adv_long_form SET monthly = '".$result[0]['ranking']."' WHERE  adv_id = '".$result[0]['brand_adv_id']."'";
            $stmt = $this->db->prepare($updateMonthlykRanking);
            $stmt->execute();
        }else{
            $updateMonthlykRanking = "UPDATE report_adv_long_form SET monthly = '-' WHERE adv_id ='".$adv_id."'";
            $stmt = $this->db->prepare($updateMonthlykRanking);
            $stmt->execute();
        }
    }
    ...
    public function addToTable($data){
        ...
        $sql =  "INSERT INTO report_adv_long_form (adv_id,from_date,to_date,spend_index,total_airings,price,network,asd,video_id) VALUES ('".$data['ID']."','".$this->last_week['sd']."','".$this->last_week['ed']."','".$data['spend_index']."','".$data['no_of_airings']."','".$data['projected_score']."','".$data['network']."','".$asd."','".$video_id."')";
       $stmt = $this->db->prepare($sql);
       $stmt->execute();
    }
    
    public function updateRanking(){
        $sql = "SELECT * FROM report_adv_long_form WHERE from_date = '".$this->last_week['sd']."' AND to_date = '".$this->last_week['ed']."' ORDER BY  CAST(spend_index as DECIMAL(10,2)) DESC";
        $stmt = $this->db->prepare($sql);
       if ($stmt->execute()) {
        ...
        foreach($rankingBrands as $key => $value){
              if($value['spend_index']!=0){
                $updateRanking = "UPDATE report_adv_long_form SET current_week = '".($key+1)."' WHERE  adv_id = '".$value['adv_id']."'";
                $stmt = $this->db->prepare($updateRanking);
                $stmt->execute();
              }else{
                $updateRanking = "UPDATE report_adv_long_form SET current_week = '-' WHERE  adv_id = '".$value['adv_id']."'";
                $stmt = $this->db->prepare($updateRanking);
                $stmt->execute();
            }
        }
    ...
    function updateAdvertiser(){
        $sql = "SELECT a.adv_id,a.company_name FROM advertiser a INNER JOIN report_adv_long_form r ON r.adv_id = a.adv_id";
         $stmt = $this->db->prepare($sql);
        $stmt->execute();
        $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
        if(!empty($result)){
            foreach($result as $k=>$v){
              $updateAdvertiserQuery = "UPDATE report_adv_long_form SET advertiser_name = '".addslashes(trim($v['company_name']))."' WHERE  adv_id = '".$v['adv_id']."'";
    ...
    function getLongBrands(){
        $report = array();
        $sql = "SELECT * FROM report_adv_long_form WHERE from_date = '".$this->last_week['sd']."' AND to_date = '".$this->last_week['ed']."' ORDER BY  CAST(spend_index as DECIMAL(10,2)) DESC LIMIT 50";
        $stmt = $this->db->prepare($sql);
        $stmt->execute();
        $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
        if(!empty($result)){
        ...
        $html = '<style>
            ...
            <div class="main-report">
				...
				<table class="table-info">
					...
					<tbody>';
         if(empty($result)){
             $html.= '<tr><td colspan="10">No records found.</tr>';
        }else{
	foreach($result as $k => $v){
            ...
            $html .='<tr>
                                                    ...
                                                    <td>'.$v['asd'].'</td>
                                                    <td>'.$v['network'].'</td>
                                                    <td><a href="http://'.HOST.'/drmetrix/video/'.$v['video_id'].'?pdf=1"><img src="http://'.HOST.'/drmetrix/assets/img/icon-play.png" /></td>
                                            </tr>';
                                            }
        }
                       $html .='      </tbody>
    ...
    function convertToPdf($html){
        ...
        $file_to_save = '../assets/pdf/Drmetrix_weekly_advertise_longform_'.date("jS_M", strtotime($this->last_week['sd'])).'.pdf';
        file_put_contents($file_to_save, $output);

    }
     
    function sendWeeklyReport(){
        $file_name =  'Drmetrix_weekly_advertise_longform_'.date("jS_M", strtotime($this->last_week['sd'])).'.pdf';
        $type = 'Advertise';
        $form = 'Long Form';
